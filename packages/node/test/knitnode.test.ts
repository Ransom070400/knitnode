...
test('saving commits a new generation and retires the old one', () => {
  withTempDir((dir) => {
...
test('a save interrupted before the manifest swap leaves the old checkpoint intact', () => {
  withTempDir((dir) => {
    ...
    // And the next save clears the abandoned generation rather than trusting it.
    node.saveCheckpoint();
